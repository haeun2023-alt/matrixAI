...
- `역행렬 없음(det=0)` 을 고르고 되돌리기를 켜 보세요. 왜 실패할까요?
    ...
    # ---------------------------------------------------------- 대칭 갤러리
    st.divider()
    st.subheader("대칭변환 네 가지 한눈에 보기")
    st.caption("책자 31쪽 『대칭변환의 행렬』을 한 화면에 모았습니다. "
               "회색은 원래 도형(F), 파란색은 대칭이동한 결과입니다. "
               "네 행렬의 det를 비교해 보세요.")

    SYM = [
        ("x-axis",   np.array([[1, 0], [0, -1]], float),
         r"x\text{축}", r"\begin{pmatrix}1&0\\0&-1\end{pmatrix}"),
        ("y-axis",   np.array([[-1, 0], [0, 1]], float),
         r"y\text{축}", r"\begin{pmatrix}-1&0\\0&1\end{pmatrix}"),
        ("origin",   np.array([[-1, 0], [0, -1]], float),
         r"\text{원점}", r"\begin{pmatrix}-1&0\\0&-1\end{pmatrix}"),
        ("y=x",      np.array([[0, 1], [1, 0]], float),
         r"y=x", r"\begin{pmatrix}0&1\\1&0\end{pmatrix}"),
    ]

    figg, axg = plt.subplots(1, 4, figsize=(12, 3.2))
    for ax, (label, Ms, _, _) in zip(axg, SYM):
        Qs = F @ Ms.T
        poly0 = np.vstack([F, F[0]])
        poly1 = np.vstack([Qs, Qs[0]])
        ax.fill(poly0[:, 0], poly0[:, 1], color="#94a3b8", alpha=0.30)
        ax.plot(poly0[:, 0], poly0[:, 1], color="#94a3b8", lw=1.5)
        ax.fill(poly1[:, 0], poly1[:, 1], color="#2563eb", alpha=0.40)
        ax.plot(poly1[:, 0], poly1[:, 1], color="#2563eb", lw=2)
        if label == "y=x":
            ax.plot([-1.4, 1.4], [-1.4, 1.4], color="#f59e0b", lw=1, ls="--")
        ax.axhline(0, color="#334155", lw=0.8)
        ax.axvline(0, color="#334155", lw=0.8)
        ax.set_xlim(-1.4, 1.4); ax.set_ylim(-1.4, 1.4)
        ax.set_aspect("equal"); ax.grid(True, alpha=0.25)
        ax.set_title(f"{label}   (det = {det2(Ms):+.0f})", fontsize=10)
    ...
    gcols = st.columns(4)
    for gc, (_, Ms, kor, tex) in zip(gcols, SYM):
        gc.latex(rf"{kor}\ \text{{대칭}}")
        gc.latex(tex)
        gc.caption(f"det = {det2(Ms):+.0f}")

    st.info("🔎 **찾았나요?** x축·y축·y=x 대칭은 det = −1 (거울처럼 **뒤집힘**), "
            "**원점 대칭만 det = +1**. 원점 대칭은 사실 **180° 회전**과 같은 행렬이라 "
            "뒤집히지 않고 돌아가기 때문입니다. 탭2 위쪽에서 회전각을 180°로 맞춰 비교해 보세요.")
